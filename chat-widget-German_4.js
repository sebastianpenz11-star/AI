<script>
// Chat Widget Script (mit "Denkt..."-Indicator & Wiggle)
(function() {
    ...
    const styles = `
        ...
        .n8n-chat-widget .close-button {
            ...
            transition: opacity 0.2s; font-size: 50px; opacity: 0.6;
        }
        ...
        /* 🧠 "Denkt..." / Typing Indicator */
        .n8n-chat-widget .chat-message.thinking {
            display: inline-flex; align-items: center; gap: 8px;
            background: var(--chat--color-background);
            border: 1px dashed rgba(133, 79, 255, 0.35);
            color: var(--chat--color-font);
            opacity: 0.9;
        }
        ...
        .n8n-chat-widget .thinking-dots span {
            width: 6px; height: 6px; border-radius: 50%;
            background: currentColor; opacity: 0.35; display: inline-block;
            animation: thinking-bounce 1.2s infinite ease-in-out;
        }
        .n8n-chat-widget .thinking-dots span:nth-child(2) { animation-delay: 0.15s; }
        .n8n-chat-widget .thinking-dots span:nth-child(3) { animation-delay: 0.3s; }
        ...
        .n8n-chat-widget .chat-input button {
            ...
            transition: transform 0.2s, opacity 0.2s; font-family: inherit; font-weight: 500;
        }
        .n8n-chat-widget .chat-input button[disabled] { opacity: 0.6; cursor: not-allowed; }
        ...
        .n8n-chat-widget .chat-toggle.wiggle {
    ...
    let thinkingEl = null;
    function showThinking(label = 'Der Assistent denkt') {
        if (thinkingEl) return;
        thinkingEl = document.createElement('div');
        thinkingEl.className = 'chat-message bot thinking';
    ...
    // 💃 Wiggle Helper (alle 12s kurzer Wackler, pausiert wenn Chat offen)
    function scheduleWiggle() {
        clearInterval(wiggleTimer);
        wiggleTimer = setInterval(() => {
            if (!chatContainer.classList.contains('open')) {
                toggleButton.classList.add('wiggle');
                setTimeout(() => toggleButton.classList.remove('wiggle'), 900);
            }
        }, 12000);
    }
    scheduleWiggle();
    ...
    async function startNewConversation() {
        ...
        try {
            ...
            // Thinking anzeigen, während die Welcome-Message geladen wird
            showThinking('Lade Willkommensnachricht');
            ...
            // 🧠 Bot-Antwort mit HTML-Unterstützung
            ...
            errDiv.textContent = 'Sorry, etwas ist schiefgelaufen. Bitte versuche es erneut.';
    ...
    async function sendMessage(message) {
        ...
        // UI sperren & Thinking zeigen
        sendButton.disabled = true;
        textarea.disabled = true;
        showThinking('Der Assistent denkt');
        ...
        try {
            ...
            // 🧠 Bot Message mit HTML-Unterstützung
            const botMessageDiv = document.createElement('div');
            botMessageDiv.className = 'chat-message bot';
            ...
            const errDiv = document.createElement('div');
            errDiv.className = 'chat-message bot';
            errDiv.textContent = 'Fehler beim Senden. Bitte versuche es noch einmal.';
            messagesContainer.appendChild(errDiv);
        } finally {
    ...
    toggleButton.addEventListener('click', () => {
        ...
        // Wenn Chat geöffnet ist, kurz wackeln stoppen (visuelles Rauschen reduzieren)
        if (chatContainer.classList.contains('open')) {
